...
ngx_http_spdy_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->headers_out.server == NULL) {
        if (clcf->server_tag_type == NGX_HTTP_SERVER_TAG_ON) {
            ...
            len += clcf->server_tokens ? ngx_http_spdy_nv_vsize(ZIONX_VER)
                                       : ngx_http_spdy_nv_vsize(ZIONX);
    ...
    if (r->headers_out.server == NULL) {
        if (clcf->server_tag_type == NGX_HTTP_SERVER_TAG_ON) {
            ...
            last = clcf->server_tokens
                   ? ngx_http_spdy_nv_write_val(last, ZIONX_VER)
                   : ngx_http_spdy_nv_write_val(last, ZIONX);
            count++;
...
ngx_http_spdy_v3_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->headers_out.server == NULL) {
        if (clcf->server_tag_type == NGX_HTTP_SERVER_TAG_ON) {
            ...
            len += clcf->server_tokens ? ngx_http_spdy_v3_nv_vsize(ZIONX_VER)
                                       : ngx_http_spdy_v3_nv_vsize(ZIONX);
        } else if (clcf->server_tag_type == NGX_HTTP_SERVER_TAG_CUSTOMIZED) {
            len += ngx_http_spdy_v3_nv_nsize("server");
    ...
    if (r->headers_out.server == NULL) {
        if (clcf->server_tag_type == NGX_HTTP_SERVER_TAG_ON) {
            ...
            last = clcf->server_tokens
                   ? ngx_http_spdy_v3_nv_write_val(last, ZIONX_VER)
                   : ngx_http_spdy_v3_nv_write_val(last, ZIONX);
            count++;
